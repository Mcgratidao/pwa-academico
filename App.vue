...
// AJUSTE 2: Datas Cinzas apenas quando uma matéria for selecionada
const datasDesativadas = computed(() => {
  if (zonaAtiva.value === 'academico' && materiaSelecionada.value) {
    ...
    // Bloqueia todos os dias que não são o dia da aula (V-Calendar usa 1-7 onde 1=Dom)
...
// AJUSTE 3: Lógica de Bloqueio de Clique no Calendário
const abrirModal = (day) => {
  // Se for Acadêmico e estiver no Geral, não abre modal
  if (zonaAtiva.value === 'academico' && !materiaSelecionada.value) return;

  // Se for Acadêmico e tiver matéria selecionada, só abre se o dia bater
  if (zonaAtiva.value === 'academico' && materiaSelecionada.value) {
...
// MÉTODOS (Mantidos iguais aos seus)
...
<style scoped>
/* ESTILOS (Mantidos conforme sua versão favorita) */
